...
var (
	...
	PasswordFormatErr      = NewParamError("密码格式错误")
	IncorrectPasswordErr   = NewUnauthorizaedError("密码错误")
	RestPwdNotSameErr      = NewParamError("两次输入的密码不一致")
	NewPwdIsSameErr        = NewParamError("新旧密码不能一致")
	NewPwdBelowIntervalErr = NewParamError("两次修改密码的间隔不得低于7天")
	...
	InvalidEmailAddrErr    = NewParamError("无效的邮箱地址")
	PermissionDeniedErr    = NewError(PermissionDenied, "权限不足")
	NicknameExistErr       = NewParamError("用户昵称已存在")
	DuplicakeErr           = NewInternalError("请勿重复申请")
	UserNotSameErr         = NewError(PermissionDenied, "用户不一致")
)
